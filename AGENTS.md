...
7. **写清注释（含 Step）**  
   按第 10 节「测试文件中的注释规范」在文件中写清注释：类与测试方法必有 docstring；**测试方法 docstring 中必须列出 Step 1、Step 2、…，下方代码严格按 Step 分段编写**，每段以 `# Step N: ...` 对应，关键逻辑与断言按需加行内注释。
...
def test_case2_both_provided_max_fee_amount_tighter_succeeds(self):
    """
    Case 2: max_fee_amount 更严，但设置为实际所需费用时，支付应成功。
    Step 1: dry_run 获取本笔支付所需费用。
    Step 2: 以 max_fee_amount 更严、max_fee_rate 放宽松发起真实支付。
    Step 3: 等待支付成功。
    Step 4: 断言实际费用未超过 max_fee_amount。
    """
```
...
def test_boundary_max_fee_rate_zero_rejects_payment(self):
    """
    边界：max_fee_rate=0 时应拒绝支付。
    Step 1: 以 max_fee_rate=0 调用 send_payment（dry_run）。
    Step 2: 断言错误信息与费用/路由相关。
    """
    amount = 1 * 100000000
    # Step 1: 以 max_fee_rate=0 调用 send_payment（dry_run）
    with pytest.raises(Exception) as exc_info:
    ...
    # Step 2: 断言错误信息与费用/路由相关
    err = exc_info.value.args[0] if exc_info.value.args else ""
    assert "Failed" in err or "max_fee" in err.lower(), f"实际错误: {err}"
```
...
```python
# max_fee_rate=1 表示 0.1%，费用上限 = amount * 1 // 1000
expected_fee_limit = amount * max_fee_rate // 1000
# ...
assert actual_fee <= expected_fee_limit, "实际费用应不超过按费率计算的上限"
```
...
- 使用中文或英文均可，同一文件内统一。
- 注释要写「做什么、为何这样做」，避免只重复代码（如不要写 `# 调用 send_payment` 而下面只有 `send_payment(...)`，应写成「以 dry_run 获取预估费用，用于后续设置 max_fee_amount」等有信息量的说明）。
- skip 的用例：在 `@pytest.mark.skip("原因")` 或方法 docstring 中写清为何跳过、计划何时修复或是否长期跳过。
...
def _parse_fee(fee) -> int:
    """解析 fee 字段，支持 hex 字符串或整数，返回 msat。"""
    if fee is None:
        return 0
...
class TestFee(SharedFiberTest):
    ...
    send_payment 的 max_fee_rate / max_fee_amount 校验与默认行为。
    需求: https://github.com/nervosnetwork/fiber/pull/1073
    拓扑: fiber1→2→3→…→8，用于多跳支付与费用校验。
    """
    fiber3: Fiber
    # ...

    def setUp(self):
        """仅初始化一次多跳拓扑，避免每个用例重复建链。"""
        if getattr(TestFee, "_channel_inited", False):
            return
        TestFee._channel_inited = True
        self.__class__.fiber3 = self.start_new_fiber(self.generate_account(10000))
        # 构建 fiber1→2→3→…→8 的线性通道，使用语义化常量
        self.open_channel(
            self.fiber1, self.fiber2,
            fiber1_balance=Amount.ckb(1000),
            fiber2_balance=0,
    ...
    def test_case2_both_provided_max_fee_amount_tighter_succeeds(self):
        ...
        max_fee_amount = required_fee + 10000  # 略大于实际费用，保证通过
        ...
        payment = self.fiber1.get_client().send_payment({
            ...
            "amount": hex(amount),
            "keysend": True,
            "max_fee_rate": hex(PaymentFeeRate.HIGH),  # 足够大的支付路由费率
        ...
        assert _parse_fee(actual.get("fee")) <= max_fee_amount, "费用应不超过设定的上限"

    def test_boundary_max_fee_rate_zero_rejects_payment(self):
        """
        边界：max_fee_rate=0 时应拒绝支付（不允许任何费用）。
        Step 1: 以 max_fee_rate=0 调用 send_payment（dry_run）。
        Step 2: 断言错误信息与费用/路由相关。
        """
        amount = Amount.ckb(1)  # 1 CKB
        ...
        with pytest.raises(Exception) as exc_info:
            self.fiber1.get_client().send_payment({
                ...
                "amount": hex(amount),
                "keysend": True,
                "max_fee_rate": hex(FeeRate.ZERO),  # 零费率
                "dry_run": True,
            })
        
        # Step 2: 断言错误信息与费用/路由相关
        err = exc_info.value.args[0] if exc_info.value.args else ""
        assert "Failed" in err or "max_fee" in err.lower(), f"max_fee_rate=0 应拒绝支付，实际: {err}"

    def test_payment_with_assertions(self):
        """
        演示使用断言辅助函数进行测试。
        Step 1: 发送支付。
        Step 2: 使用断言辅助函数验证（支付成功、通道状态）。
        """
