"""
Test open_channel commitment_delay_epoch: epoch-based delay and self/remote shutdown.
"""
import time
...
class TestCommitmentDelayEpoch(FiberTest):
    """
    Test open_channel with commitment_delay_epoch: epoch delay and balance after shutdown.
    """
    ...
    def test_epoch(self):
        """
        Open channel with commitment_delay_epoch, force shutdown, generate epochs, assert tx capacity change.
        Step 1: Open channel with commitment_delay_epoch, wait CHANNEL_READY, force shutdown.
        Step 2: Wait for tx pool, commit tx, generate epochs.
        Step 3: Assert first and second tx input capacity difference equals 100 CKB.
        """
        # Step 1: Open channel with commitment_delay_epoch, wait CHANNEL_READY, force shutdown
        ...
        # Step 3: Assert first tx input capacity and difference
    ...
    def test_self_shutdown(self):
        """
        Open channel with commitment_delay_epoch and tlc_expiry_delta, self force shutdown, assert balance.
        Step 1: Open channel, wait CHANNEL_READY, force shutdown from fiber1.
        Step 2: Commit tx, generate epochs, wait until commit cells clear.
        Step 3: Assert both parties have small positive CKB balance change.
        """
    ...
    def test_remote_shutdown(self):
        """
        Open channel with commitment_delay_epoch, remote (fiber2) force shutdown, assert balance.
        Step 1: Open channel, wait CHANNEL_READY, force shutdown from fiber2.
        Step 2: Commit tx, generate epochs, wait until commit cells clear.
        Step 3: Assert both parties have small positive CKB balance change.
        """
        before_balance = self.get_fibers_balance()
